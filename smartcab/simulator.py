...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False,
		 epsilon = 1, alpha = 0.5, desvio = 0, gamma = 0, ajuste = 0.95):
        self.env = env
        ...
        if self.log_metrics:
		    ...
		    NomeArquivo = "_epsilon" + str(epsilon) + "_alpha" + str(alpha) + "_desv" + str(desvio) + "_gamma" + str(gamma) + "_ajuste" + str(ajuste) 
                    ...
                    self.log_filename = os.path.join("logs", 'sim_improved-learning_' + NomeArquivo + ".csv")

                    self.table_filename = os.path.join("logs",'sim_improved-learning_' + NomeArquivo + ".txt")
                else: 
